...
function scheduleNotifications(wakeupWeekday, sleepWeekday, wakeupWeekend, sleepWeekend){
	...
	var interval1, interval2, interval3, interval4, interval5, interval6, interval7, interval8;
	var a, b, c, d, e, f, g, h;
	...
	for (i = 0; i < 14; i++) {
		...
		step = parseInt(awakeInterval / 7);


		//Then time for notifications is calculated as a random time within the range of + or - 30 minutes of the anchors. It is represented as the interval between the current time and the time of that notification
		interval1 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000));
        interval2 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step;
        interval3 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*2;
        interval4 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*3;
        interval5 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*4;
        interval6 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*5;
        interval7 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*6;
        interval8 = (parseInt(nightlyLag) + 30*60*1000 + parseInt(Math.round((Math.random()*2 - 1)*30*60*1000)) + ((parseInt(86400)*parseInt(i))*1000)) + step*7;
        ...
        d6 = date6.getFullYear()+"-"+(date6.getMonth()+1)+"-"+date6.getDate()+" "+date6.getHours()+":"+date6.getMinutes()+":"+date6.getSeconds();
        d7 = date7.getFullYear()+"-"+(date7.getMonth()+1)+"-"+date7.getDate()+" "+date7.getHours()+":"+date7.getMinutes()+":"+date7.getSeconds();
        d8 = date8.getFullYear()+"-"+(date8.getMonth()+1)+"-"+date8.getDate()+" "+date8.getHours()+":"+date8.getMinutes()+":"+date8.getSeconds();
        ...
        cordova.plugins.notification.local.schedule({icon: 'ic_launcher', id: f, trigger: {at: date6}, text: 'Time for your next Survey!', title: "Hello, I'm back"});
        cordova.plugins.notification.local.schedule({icon: 'ic_launcher', id: g, trigger: {at: date7}, text: 'Time for your next Survey!', title: "Hello, I'm back"});
        cordova.plugins.notification.local.schedule({icon: 'ic_launcher', id: h, trigger: {at: date8}, text: 'Time for your next Survey!', title: "Hello, I'm back"});

        data += "&n"+a+"="+d1+"&n"+b+"="+d2+"&n"+c+"="+d3+"&n"+d+"="+d4+"&n"+e+"="+d5+"&n"+f+"="+d6+"&n"+g+"="+d7+"&n"+h+"="+d8;
	}
	return(data);
}
